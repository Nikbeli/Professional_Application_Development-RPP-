...
namespace FurnitureAssemblyDatabaseImplement.Implements
{
	public class WorkPieceStorage : IWorkPieceStorage
	{
		public List<WorkPieceViewModel> GetFullList()
		{
			...
			return context.WorkPieces.Select(x => x.GetViewModel).ToList();
		}
		...
		public List<WorkPieceViewModel> GetFilteredList(WorkPieceSearchModel model)
		{
			...
			return context.WorkPieces.Where(x => x.WorkPieceName.Contains(model.WorkPieceName))
		...
		public WorkPieceViewModel? GetElement(WorkPieceSearchModel model)
		{
			...
			return context.WorkPieces.FirstOrDefault(x => (!string.IsNullOrEmpty(model.WorkPieceName)
				&& x.WorkPieceName == model.WorkPieceName) || (model.Id.HasValue && x.Id == model.Id))?.GetViewModel;
		}
