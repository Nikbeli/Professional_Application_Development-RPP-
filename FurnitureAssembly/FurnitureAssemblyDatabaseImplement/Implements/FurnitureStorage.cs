...
namespace FurnitureAssemblyDatabaseImplement.Implements
{
	public class FurnitureStorage : IFurnitureStorage
	{
		public List<FurnitureViewModel> GetFullList()
		{
			...
			return context.Furnitures.Include(x => x.WorkPieces)
				.ThenInclude(x => x.WorkPiece).ToList().Select(x => x.GetViewModel).ToList();
		}
		...
		public List<FurnitureViewModel> GetFilteredList(FurnitureSearchModel model)
		{
			...
			return context.Furnitures.Include(x => x.WorkPieces).ThenInclude(x => x.WorkPiece)
				.Where(x => x.FurnitureName.Contains(model.FurnitureName)).ToList()
		...
		{
			using var context = new FurnitureAssemblyDatabase();
			var element = context.Furnitures.Include(x => x.WorkPieces)
				.Include(x => x.Orders).FirstOrDefault(rec => rec.Id == model.Id);
